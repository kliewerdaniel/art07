...
export default function ArtistDashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const artworksData = await artworks.getByArtist(user?.id || '')
      setRecentArtworks(artworksData.items.slice(0, 3) as unknown as Artwork[])

      // Load user's donations
      const donationsData = await donations.getByArtist(user?.id || '')
      ...
      setStats({
        ...
        mentorshipRequests: 0, // TODO: Load from mentorship requests
        assessments: 0 // TODO: Load from assessments
      })
